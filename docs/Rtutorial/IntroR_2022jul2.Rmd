...
## Welcome to this mini Intro to R workshop!
My goal is to introduce you to some tools and new knowledge that will allow you to advance your understanding of biological research by practicing the most commonly used programming language in computational biology: R. You can think of learning **R** as learning how to use a smartphone, and learning the specific **R packages** as the various apps designed to help you do powerful scientific analyses. So today, we will familiarize with this important ***smartphone*** and introduce a couple of ***apps***.

### Step 1: Create an account with RStudio Cloud
**R & R Studio are free!**
For this mini workshop, we will be using R Studio Cloud. To create a free account just access this website *https://login.rstudio.cloud* and create a new account using your preferred email address. For more learning resources on how to  use R Studio, check out these guidelines written by Thomas Mock: *https://www.rstudio.com/resources/webinars/a-gentle-introduction-to-tidy-statistics-in-r/*.


### *Alternative*: Step 1b: Download and install R + R studio 
...
### Step 2: Learning the Help Command

***Practice makes perfect***
A way to learn more R is by deciphering error messages! 
So, if you get an error message, which is the output showing on red font, don't panic! Sometimes the best way to learn R is figuring out what the error messages mean and why do we get them. The easiest way to do this is to **google** your error message & most likely someone has already found a solution - there is literally a ton of help online! Now since we are learning how to code, we can ask R for help using a specific function in a package by calling in R's built-in help function, or by directly typing a question mark before the function you are trying to use.
...
### Data management using data frames
...
### Publication-ready plots using ggplot2 and dplyr
Base R plots can be modified to look as pretty/detailed as you would like them to look. However, packages like **ggplot2** make all the heavy-lifting for you. Next we will learn how to make a few simple, yet publication-level, *nice looking plots* using **ggplot2** and **dplyr**.
...
#Let's explore other functions and plots in the package explore
...
#We will then have to load tibble and explore
library(tibble)
library(explore)
...
```
In this example we went beyond simply plotting and we actually combined data re-organization with visualization, and figure export using the packages *dplyr*, *ggplot2*, *tibble*, *explore*, and *patchwork*.
...
### Now we will end with a familiar statistical test.
